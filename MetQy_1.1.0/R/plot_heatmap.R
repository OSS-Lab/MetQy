...
#'
#' @param data_in     - numeric matrix OR 3-column data frame. See \code{Details}.
#'
...
#'
#' @param ORDER_MATRIX - logical. Should rows and columns be reorder according to the dendogram (hierarchical clustering). Default (\code{TRUE}).
#'
...
#'
#' If \code{data_in} is a numeric matrix, it will be 'melted' into this type of data frame by using the \code{reshape2::melt} function.
#'
...
plot_heatmap <- function(data_in, row_lab = "Genomes", col_lab = "Modules",ORDER_MATRIX = TRUE,legend_name = "Module\ncompleteness\nfraction\n",
  ...
  p <-
    ...
    ggplot2::theme(axis.ticks = ggplot2::element_blank(),
                  axis.title.y	= ggplot2::element_text(size = Width*1.3,margin = ggplot2::margin(t = 0, r = 10, b = 0, l = 0)),
                  axis.title.x	= ggplot2::element_text(size = Width*1.3,margin = ggplot2::margin(t = 10, r = 0, b = 0, l = 0))) +
  ...
  if(length(unique(DATA[,2]))>50){
    ...
    p <- p + ggplot2::theme(axis.text.x   = ggplot2::element_text(hjust = 1,vjust=1,size=Width*1.2, colour = "black",angle = 45)) +
              ggplot2::theme(plot.margin  = ggplot2::unit(c(0.5,0.5,0.5,0.5+nChars/3.5),"cm"))   # top, right, bottom, and left
  }
