...
#'
#' @seealso \link{misc_geneVector_module}, \link{analysis_genomes_module_output}, \link{plot_heatmap},
#'
#' @seealso \link{data_example_multi_ECs_KOs}
#'
#' @examples
#' ## USE T numbers
  ...
  if(is.character(GENOME_INFO)){
    ...
    if(length(ref_index)>0){
      ...
      if(length(no_info_index)>0){
        warning("The following organisms have no KO listed in KEGG genome:")
        for(R in no_info_index) cat(paste(GENOME_INFO_DATA[R,],collapse = "\t"),fill = T)
