...
#'
#' @param Filename    - optional. Character vector containing the file path(s) to save image(s). Must have an extension. 
      ...
      if(is.character(this_factor)&&length(this_factor)==nrow(MATRIX)){
        ...
        if(Filename!=""&&is.character(Filename)){
          ...
          for(FF in File){
            ...
            plot(MATRIX,xlab = "PC1",ylab = "PC2",pch = 16, col = "grey80") # PC1 v PC2
            ...
            if(nGroups > 1) legend(max(MATRIX[,1])*1.075,max(MATRIX[,2])*1.1,legend = group_legend,pch = 16,col = COLOURS, horiz = F, cex = ifelse(nGroups<46,0.7,0.55), bty = "n")
            par(xpd=FALSE)
            dev.off()
