...
#'
#' Read the KEGG module database text file ("\code{KEGG_path}/module/module.gz" OR "\code{KEGG_path}/module/module")
#' and format it into a reference table.
#' Note that the columns are automatically generated by the \code{parseKEGG_file} function and
#' columns or variables specific to the KEGG module database are further formated.
#'
...
#'
#' A MODULE is made up of BLOCKS (SPACE delimitted). Each block is defined by a logical expression
...
#' Where space or a plus sign could represent an AND operation in the KEGG definition,
#' we have replaced all instances with \code{'&'} and all comas (used to represent an OR operation)
#' has been replaced with \code{'|'} in this expression.
#'
...
#' 	     (15) DBLINKS;         (16) RMODULE;         (17) REFERENCE;         (18) COMMENT;         (19) BRITE;
#'
...
#' @export
#'
#' @seealso \link{parseKEGG_file}, \link{misc_module_definition_check},\link{misc_module_definition_optional},
#' \link{misc_module_definition_block_EC}
