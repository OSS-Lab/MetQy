...
#'
#' @param plot_data - data frame. The columns to be ploted are indicated with \code{X} and \code{Y}.Order given is conserved.
#'
#' @param X,Y       - optional. Number indicating column to use for x axis and y axis, respectively. Default (1 and 3).
#'
#' @param colBy     - optional. Number indicating column to use for coloruing grouping. Default (\code{NULL}).
#'
#' @param xLab,yLab - optional. String to use for x label and y label, respectively. Default (\code{names(plot_data)[1] and [2]}, respectively).
#'
...
#' @export
#' @examples
#' plot_data_example <- data.frame("Groups"=LETTERS[1:12],"Factor"=c(rep(1,4),rep(2,4),rep(3,4)),"Value"=runif(12,-10,10),stringsAsFactors = FALSE)
