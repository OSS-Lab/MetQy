...
#'
#' Read and format the KEGG genome database text file ("\code{KEGG_path}/genes/genome.tar.gz" OR
#' "\code{KEGG_path}/genes/genome/genome")
#' and the organism information and format it into a reference table.
#' Note that the columns are automatically generated by the \code{parseKEGG_file} function and
#' columns or variables specific to the KEGG genome database are further formated.
#'
#' @param KEGG_path     - string pointing to the location of the KEGG database parent folder.
#'
#' @param outDir        - optional. String pointing to the output folder. Default ("output/"). \code{NULL} overwrites writting files.
#'
...
#'
#' @param includeVirues - logical. Should viral genomes be included? Default (\code{FALSE}).
#'
...
parseKEGG_genome <- function(KEGG_path, outDir = "output",addECs = T, addKOs = T,includeVirues = F, verbose = T,...){
  ...
  stopifnot(is.logical(addECs),is.logical(addKOs),is.logical(includeVirues),is.logical(verbose))
  ...
  # KEEP VIRAL GENOMES?
  if(!includeVirues) if(length(grep("Viral",STATUS))>0){
